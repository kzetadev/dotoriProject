...
//아이디 유효성 검사(1=중복/ 0 != 중복)
	$("#mem_id").blur(function(){0.
		...
		$.ajax({
			...
			success : function(data){
				...
				console.log("ajax 실패실패실패")
				if(data == 1){
					console.log("아이디 중복 성공")
					//1: 아이디가 중복되는 문구
	...
	$("#reg_submit").click(function(){
		consloe.log("가입하기 실행버트 유효성 검사 성공")
		//비밀번호가 같은 경우 && 비밀번호 정규식
		if(($("#mem_pwd").val() == ($("#mem_pwdcheck").val()))
					&& pwdJ.test($("#mem_pwd").val())) {
			console.log("가입하기 실행버트 유효성 검사 성공 ")
				inval_Arr[0] = true;
			}else{
				console.log("가입하기 실행버트 유효성 검사 실패 ")
				inval_Arr[0] = false;
			}
...
<div class="container">
	...
	<form method="POST" action="../pro/JoinPro.do" name="memInfo" onsubmit="return checkValue()">
	<!-- 세션에 저장한 소셜 아이디를 가져옴  -->
	...
	<!--세션에 저장한 소셜 구분을 가져옴 -->
		...
		<div class="form-froup">
			<label for="mem_name">성명</label>
				<input type="text" class="form-control" id="mem_name" name="mem_name" placeholder="성명" required>
		...
		<div class="form-group">
			<label for="mem_nickname">닉네임</label>
				<input type="text" class="form-control" id="mem_nickname" name="mem_nickname" placeholder="닉네임" required>
		...
		<div class="form-group">
			<label for="mem_id">아이디</label>
				<input type="text" class="form-control" id="mem_id" name="mem_id"placeholder="아이디" required>
		...
		<div class="form-group">
			<label for="mem_pwd">비밀번호</label>
				<input type="password" class="form-control" id="mem_pwd" name="mem_pwd" placeholder="비밀번호" required>
		...
		<div class="form-group">
			<label for="mem_pwdcheck">비밀번호 확인</label>
				<input type="password" class="form-control" id="mem_pwdcheck" name="mem_pwdcheck"  placeholder="비밀번호 확인" required>
		...
		<div class="form-group">
				<label for="mem_email">이메일</label>
					<input type="text" class="form-control" name="mem_email" id="mem_email" placeholder="E-mail" maxlength="50" required>@
						<select name="mail2">
		...
		<div class="reg_button">
				<a class="btn btn-danger px-3" href="${pageContext.request.contextPath}">
					<i class="fa fa-rotate-right pr-2" aria-hidden="true"></i>가입취소하기</a>
		</div>
