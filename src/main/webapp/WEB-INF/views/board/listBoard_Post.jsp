...
<body>
	<h2>
		<c:forEach items="${headtag }" var="i" varStatus="status" end="3">
			<c:choose>
				<c:when test="${status.index eq 1 }">자유</c:when>
				<c:when test="${status.index eq 2 }">후기</c:when>
				<c:when test="${status.index eq 3 }">동행</c:when>
